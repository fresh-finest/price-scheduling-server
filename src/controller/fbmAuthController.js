...
exports.signin = async(req,res,next)=>{
    ...
    try {
        const validUser = await FBMUser.findOne({ email: email });
...
exports.logOut = async (req, res, next) => {
  ...
  try {
    ...
    const user = await FBMUser.findById(decoded.id);
...
exports.resetPassword = async (req, res, next) => {
    ...
    try {
        const user = await FBMUser.findOne({
...
exports.storeToken = async(req,res,next)=>{
    ...
    try {
        const user = await FBMUser.findById(id);
        if(!user){
...
exports.requestPasswordReset = async (req,res,next)=>{
    ...
    try {
      ...
      const resetUrl = `https://fbm.priceobo.com/forget-password?token=${token}&email=${encodeURIComponent(user.email)}`;

      await sendFBMEmail(
        user.email.trim(),
        'Password Reset Request',
         `You are invited to set up your password using the following link: ${resetUrl}`,
         `
