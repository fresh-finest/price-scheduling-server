...
router.get("/auto-pricing-report", async (req, res) => {
  ...
  try {
    const autoScheduleResponse = await axios.get(
      `http://localhost:3000/auto-schedule`
    );
...
router.get("/auto-schedule/:sku", async (req, res) => {
  const { sku } = req.params;
  try {
    const result = await AutoSchedule.find({ sku });
